...
# from via the cascade MSP430 output build file cascade_MSP430.txt.
#
...
InputAppOutFileName = cwd + '/' + '../Cascade_MSP430/Debug/Cascade_MSP430.out'
InputBootOutFileName = cwd + '/' + '../Cascade_Boot_MSP430/Debug/Cascade_Boot_MSP430.out'
InputAppTxtFileName = cwd + '/' + '../Cascade_MSP430/Debug/Cascade_MSP430.txt'
...
p = subprocess.Popen([ 'python', 'cascadeRomToMsg.py', 'cascade_MSP430_rom.txt', 'cascade_MSP430_msg.txt' ], stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
for line in p.stdout.readlines():
    print (line)
retval = p.wait()

fileFound = os.path.exists('cascade_MSP430_msg.txt')
if fileFound == False:
    print ('cascade_MSP430_msg.txt file not created.')
    exit()
else:
    print ("File cascade_MSP430_msg.txt created.")
...
#  files
#   + Cascade_MSP430/Debug/Cascade_MSP430.txt'
...
# destination = open('cascade_App_Boot_MSP430.txt','wb')
...
for line in f:
        txtFileString+=line
f.close()
f = open('cascade_App_Boot_MSP430.txt', 'w')
f.write (txtFileString)
f.close()

fileFound = os.path.exists('cascade_App_Boot_MSP430.txt')
if fileFound == False:
    print ('cascade_App_Boot_MSP430.txt file not created.')
    exit()
else:
    print ("File cascade_App_Boot_MSP430.txt created.")
