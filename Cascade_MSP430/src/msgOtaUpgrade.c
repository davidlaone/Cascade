...
static void sendModemBatchCmd(modemBatchCmdType_t cmdType) {
    if (cmdType == MODEM_BATCH_CMD_GET_OTA_PARTIAL) {
        ...
        otaUpData.cmdWrite.cmd = M_COMMAND_GET_INCOMING_PARTIAL;
...
static bool otaUpgrade_eraseSection(void) {
    ...
    if (otaUpData.sectionDataRemaining > OTA_PAYLOAD_BUF_LENGTH) {
        otaUpData.modemRequestLength = OTA_PAYLOAD_BUF_LENGTH;
    } else {
