...
typedef struct otaData_s {
    ...
    uint8_t rebootKeys[4];         /**< If reboot required by msg, must be set correctly to reboot */
...
static bool otaMsgMgr_processSetFlowrateConstants(otaResponse_t *otaRespP);
static bool otaMsgMgr_processClockRequest(otaResponse_t *otaRespP);
static bool otaMsgMgr_processStatusRequest(otaResponse_t *otaRespP);
...
static void postOtaMessageProcessing(void) {
    ...
    if (otaData.gmtTimeUpdateCandidate) {
        ...
        // There are a two commands that require rebooting after OTA message
        // processing.  This includes a successful firmware upgrade message and
        // the reboot OTA message.  After all OTA processing is done, check if
        // we need to reboot.
        if ((otaData.rebootKeys[0] == REBOOT_KEY1) &&
            (otaData.rebootKeys[1] == REBOOT_KEY2) &&
            (otaData.rebootKeys[2] == REBOOT_KEY3) &&
            (otaData.rebootKeys[3] == REBOOT_KEY4)) {
            sysExec_startRebootCountdown(otaData.rebootKeys);
            otaData.rebootKeys[0] = 0;
            otaData.rebootKeys[1] = 0;
            otaData.rebootKeys[2] = 0;
            otaData.rebootKeys[3] = 0;
        }
    }
}
...
#ifdef FOR_TEST_ONLY
        // For testing, set alignment two minutes in the future
...
static bool otaMsgMgr_processSetTransmissionRate(otaResponse_t *otaRespP) {
    ...
    if (transmissionRateInDays == 0 || transmissionRateInDays > (7 * 7)) {
        // Add OTA response data
        *responseDataP++ = 0xff; // error status
...
static bool otaMsgMgr_processResetDevice(otaResponse_t *otaRespP) {
    uint8_t *responseDataP = &otaData.responseBufP[RESPONSE_DATA_OFFSET];
    otaData.rebootKeys[0] = otaRespP->buf[3];
    otaData.rebootKeys[1] = otaRespP->buf[4];
    otaData.rebootKeys[2] = otaRespP->buf[5];
    otaData.rebootKeys[3] = otaRespP->buf[6];
...
* \li 20480 bytes of firmware   (20K, 0x5000 bytes)
*  
* \brief Output OTA response
* \li msg opcode (1 byte)
* \li msg id     (2 bytes)
...
* \li calculated crc16 of firmware (2 bytes)
*  
* @param otaRespP Pointer to the response data and other info
*                 received from the modem.
*
...
static bool otaMsgMgr_processFirmwareUpgrade(otaResponse_t *otaRespP) {
    ...
    *responseDataP++ = crc & 0xFF;
    ...
    if (fwUpdateResult == RESULT_DONE_SUCCESS) {
        ...
        // Set the reboot keys to allow a reboot to start after
        // all other OTA message processing is complete.
        otaData.rebootKeys[0] = REBOOT_KEY1;
        otaData.rebootKeys[1] = REBOOT_KEY2;
        otaData.rebootKeys[2] = REBOOT_KEY3;
        otaData.rebootKeys[3] = REBOOT_KEY4;
...
/**
* \brief (msgId=0x30) Return status information
* 
* \brief Input OTA parameters
* \li msg opcode (1 byte)
* \li msg id     (2 bytes)
...
*/
static bool otaMsgMgr_processStatusRequest(otaResponse_t *otaRespP) {
    uint16_t slopeRead = 0;
    int16_t offsetRead = 0;
    uint8_t *responseDataP = &otaData.responseBufP[RESPONSE_DATA_OFFSET];
    // Prepare OTA response.  It will be sent after this function exits.
    prepareOtaResponse(otaRespP->buf[0], otaRespP->buf[1], otaRespP->buf[2], NULL, 0);
    // Retrieve constants from flash
    flashParam_getFlowConstants(&slopeRead, &offsetRead);

    // Add OTA response status
    *responseDataP++ = 1;                  // success status
    // Add last reset reason
    *responseDataP++ = getLastRebootReason();
    // Add water constants
    *responseDataP++ = slopeRead >> 8;     // slope MSB
    *responseDataP++ = slopeRead & 0xFF;   // slope LSB
    *responseDataP++ = offsetRead >> 8;    // offset MSB
    *responseDataP++ = offsetRead & 0xFF;  // offset LSB
    // Add Red flag status
    *responseDataP++ = storageMgr_getRedFlagConditionStatus();

    return true;
}

/**
* \brief (msgId=0x31) Process Set Flowrate Constants OTA command
*
* \brief Input OTA parameters
* \li msg opcode (1 byte)
* \li msg id     (2 bytes)
...
*/
static bool otaMsgMgr_processSetFlowrateConstants(otaResponse_t *otaRespP) {
    uint16_t slopeWrite = 0;
    int16_t offsetWrite = 0;
    uint16_t slopeRead = 0;
    int16_t offsetRead = 0;
    uint8_t *responseDataP = &otaData.responseBufP[RESPONSE_DATA_OFFSET];
    // retrieve 16 bit slope value from message
    slopeWrite = (otaRespP->buf[3] << 8) | otaRespP->buf[4];
    // retrieve 16 bit offset value from message
    offsetWrite = (otaRespP->buf[5] << 8) | otaRespP->buf[6];
    // Set constants in flash
    flashParam_setFlowConstants(slopeWrite, offsetWrite);
    // Prepare OTA response.  It will be sent after this function exits.
    prepareOtaResponse(otaRespP->buf[0], otaRespP->buf[1], otaRespP->buf[2], NULL, 0);
    // Retrieve constants from flash
    flashParam_getFlowConstants(&slopeRead, &offsetRead);
    // Add OTA response data
    if ((slopeRead == slopeWrite) && (offsetRead == offsetWrite)) {
        *responseDataP++ = 1;               // success status
    } else {
        *responseDataP++ = 0xFF;            // error status
    }
    *responseDataP++ = slopeWrite >> 8;     // slope MSB
    *responseDataP++ = slopeWrite & 0xFF;   // slope LSB
    *responseDataP++ = offsetWrite >> 8;    // offset MSB
    *responseDataP++ = offsetWrite & 0xFF;  // offset LSB
    return true;
}
...
static void otaMsgMgr_processOtaMsg(void) {
    ...
    // GMT update message.  It is handled slightly differently.
    ...
    case OTA_OPCODE_SET_TRANSMISSION_RATE:
        ...
        break;
    case OTA_OPCODE_SET_FLOWRATE_CONSTANTS:
        sendOtaResponse = otaMsgMgr_processSetFlowrateConstants(otaRespP);
        break;
    case OTA_OPCODE_CLOCK_REQUEST:
        sendOtaResponse = otaMsgMgr_processClockRequest(otaRespP);
        break;
    case OTA_OPCODE_STATUS_REQUEST:
        sendOtaResponse = otaMsgMgr_processStatusRequest(otaRespP);
        break;
