 ...
 *        response from the modem will include 13 bytes of modem
...
/**
 * \def ISR_TX_BUF_SIZE 
 * \brief Define the size of the UART transmit buffer. The
...
static void initForSendTestCmd(uint8_t *payloadP, uint16_t payloadSize, uint8_t payloadMsgId);
...
static void initForModemInfoCmd(void);
...
bool modemCmd_write(modemCmdWriteData_t *writeCmdP) {
    ...
    case M_COMMAND_SEND_DATA:
        ...
        break;
    case M_COMMAND_SEND_TEST:
        initForSendTestCmd(writeCmdP->payloadP, writeCmdP->payloadLength, writeCmdP->payloadMsgId);
        break;
...
static void initForSendTestCmd(uint8_t *payloadP, uint16_t payloadSize, uint8_t payloadMsgId) {
    ...
    mcData.txHeaderP[0] = M_COMMAND_SEND_TEST;        // command byte
    mcData.txHeaderP[1] = 0;                          // Payload Size: bits 24-31
    mcData.txHeaderP[2] = 0;                          // Payload Size: bits 16-23
    mcData.txHeaderP[3] = (size >> 8) & 0xFF;         // Payload Size: bits 8-15
    mcData.txHeaderP[4] = size & 0xFF;                // Payload Size: bits 0-7
...
*/
static void initForModemInfoCmd(void) {
    mcData.modemCmdId = M_COMMAND_MODEM_INFO;
    mcData.txHeaderP[0] = M_COMMAND_MODEM_INFO;   // command Byte
    mcData.txHeaderLength = 1;
    mcData.txMsgContainsAPayload = false;
    mcData.crc = gen_crc16(&(mcData.txHeaderP[0]), mcData.txHeaderLength);  // pre-calculated crc
    mcData.expectedResponseLength = 15; // start,cmd,info[10],crc[2],end
}
