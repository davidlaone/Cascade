...
/*
TODO 
ModemLink should be renamed to modemPower 
Rename modemLinkUpStatus to modemNetworkStatus
*/
...
typedef struct mwBatchData_s {
    ...
    uint8_t imei[8];                 /**< 64bit IMEI value received from modem */
...
static void parseModemInfoCmdResponse(modemCmdReadData_t *readDataP);
...
bool modemMgr_isModemUpError(void) {
    return modemLink_isModemUpError();
}
...
/**
* \brief Return the imei as an array of eight bytes.
* 
* @param bufP The supplied buffer to store the imei bytes into. 
*/
void modemMgr_getImei(uint8_t *bufP) {
    memcpy(bufP, mwBatchData.imei, 8);
}
...
static void modemMgr_batchWriteStateMachine(void) {
    ...
    do {
        ...
        case MWBATCH_STATE_PING:
            {
                ...
                mcWriteData.cmd = M_COMMAND_MODEM_INFO;
        ...
        case MWBATCH_STATE_PING_WAIT:
            if (!modemCmd_isBusy()) {                // Read data and parse
                modemCmdReadData_t readData;
                modemCmd_read(&readData);
                parseModemInfoCmdResponse(&readData);
...
/**
* \brief Parse a modem info command response to retrieve the 
*        number of OTA messages available.
* 
* @param readDataP Pointer to a modemCmdReadData_t object 
*/
static void parseModemInfoCmdResponse(modemCmdReadData_t *readDataP) {
    if (readDataP->valid && (readDataP->modemCmdId == M_COMMAND_MODEM_INFO)) {
        // byte 0 = start byte of message
        // byte 1 = modemCmdId
        // byte 2 = major version ID
        // byte 3 = minfor version ID
        // bytes 4-12 = IMEI (a 64 bit value)
        memcpy(mwBatchData.imei, &readDataP->dataP[4], 8);
    }
}
