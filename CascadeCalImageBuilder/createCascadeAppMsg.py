#
# This utility is used to create the cascade Application upgrade message 
# from via the cascade MSP430 output build file cascade_Cal_MSP430.txt.
#
...
InputAppOutFileName = cwd + '/' + '../Cascade_Cal_MSP430/Debug/Cascade_Cal_MSP430.out'
InputBootOutFileName = cwd + '/' + '../Cascade_Boot_MSP430/Debug/Cascade_Boot_MSP430.out'
InputAppTxtFileName = cwd + '/' + '../Cascade_Cal_MSP430/Debug/Cascade_Cal_MSP430.txt'
InputBootTxtFileName = cwd + '/' + '../Cascade_Boot_MSP430/Debug/Cascade_Boot_MSP430.txt'
...
# C:\ti\ccsv5\tools\compiler\msp430_4.2.1\bin\hex430.exe cascade_app_to_rom.cmd
p = subprocess.Popen([ 'hex430.exe', 'cascade_app_to_rom.cmd' ], stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
for line in p.stdout.readlines():
    print (line)
retval = p.wait()

# Create the Outpour Upgrade Message File
p = subprocess.Popen([ 'python', 'cascadeRomToMsg.py', 'cascade_Cal_MSP430_rom.txt', 'cascade_Cal_MSP430_msg.txt' ], stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
for line in p.stdout.readlines():
    print (line)
retval = p.wait()

fileFound = os.path.exists('cascade_Cal_MSP430_msg.txt')
if fileFound == False:
    print ('cascade_Cal_MSP430_msg.txt file not created.')
    exit()
else:
    print ("File cascade_Cal_MSP430_msg.txt created.")
...
#  files
#   + Cascade_Cal_MSP430/Debug/Cascade_Cal_MSP430.txt'
#   + Cascade_Boot_MSP430/Debug/Cascade_Boot_MSP430.txt'
...
# destination = open('cascade_Cal_App_Boot_MSP430.txt','wb')
...
for line in f:
        txtFileString+=line
f.close()
f = open('cascade_Cal_App_Boot_MSP430.txt', 'w')
f.write (txtFileString)
f.close()

fileFound = os.path.exists('cascade_Cal_App_Boot_MSP430.txt')
if fileFound == False:
    print ('cascade_Cal_App_Boot_MSP430.txt file not created.')
    exit()
else:
    print ("File cascade_Cal_App_Boot_MSP430.txt created.")
